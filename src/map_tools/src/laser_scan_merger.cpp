...
// 定义时间窗口（例如：0.5秒）
    double time_window = 0.5; // 0.5秒
    rclcpp::Time current_time = this->get_clock()->now();

    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered_1(new pcl::PointCloud<pcl::PointXYZ>);

    // 过滤点云：根据时间戳过滤
    // 获取 PointCloud2 的时间戳（假设两个点云的时间戳相同）
    double cloud_time = cloud1_.header.stamp.sec + cloud1_.header.stamp.nanosec * 1e-9;

    for (const auto& point : merged_pcl_cloud.points) {
        // 计算当前点的时间戳与当前时间的时间差
        double time_diff = (current_time.seconds() - cloud_time);  // 使用当前时间减去点云时间戳

        if (time_diff <= time_window) {
            // 如果该点的时间戳在允许的时间窗口内，则保留该点
            cloud_filtered_1->points.push_back(point);
        }
    }


    // 输出过滤后的点云大小
    RCLCPP_INFO(this->get_logger(), "Filtered Point Cloud size: %zu", cloud_filtered_1->size());

    // 将合并后的点云转换回 PointCloud2
    sensor_msgs::msg::PointCloud2 merged_cloud_msg;
    pcl::toROSMsg(*cloud_filtered_1, merged_cloud_msg);
